...
//!
//! Creature conversions emit full-clone RACEs that own their subgraphs, so the
//! FO76 `SADD` (4-byte FormID of an additive-blend race) is meaningless and its
//! blind remap often dangles. Weapon/armor additive races come from
//! `generate_additive_races` and keep a correct SADD.
...
impl Fixup for StripCreatureSubgraphAdditiveRaceFixup {
    ...
    fn scope(&self) -> FixupScope {
        // Whole-plugin: self-gates per RACE below.
    ...
    fn run_with_session(
        ...
        for fk in race_fks {
            ...
            // Whole-plugin gate: humanoid and generated additive races keep a
            // valid SADD; robots and segmented creatures lose the dangling FO76
            // one. No-op on a creature-graph walk.
