...
FULL_STAGE_ORDER: dict[str, tuple[int, ...]] = {
    "Fragments:Quests:QF_W05_MQ_001P_Wayward_00405E14": (
...
EXPANDED_OBJECTIVE_STAGES = {
    "Fragments:Quests:QF_W05_MQ_001P_Wayward_00405E14": {400, 600},
    "Fragments:Quests:QF_W05_MQ_003P_Muscle_0041A39D": {
        100, 150, 200, 300, 400, 500, 600, 700, 1000, 1100, 1150, 1200,
        1300,
    },
}
...
def test_patch_preserves_the_exact_objective_bodies_and_reviewed_order(
    ...
    assert member_names == [
        _fragment_member(stage) for stage in FULL_STAGE_ORDER[script_name]
    ]
    reviewed_members = [
        _fragment_member(stage)
        for stage in positive_stages + REPAIR_CASES[script_name]
    ]
    assert [name for name in member_names if name in reviewed_members] == reviewed_members
    ...
    for stage in positive_stages:
        ...
        if stage in expanded:
            ...
            assert body.count("SetObjectiveDisplayed(") >= 1
        else:
...
def test_positive_and_full_negative_lists_cover_the_production_members(
    ...
    assert _member_names(patch) == full_members
    assert set(full_members) == set(positive_members + negative_members)
...
def test_production_merge_is_exact_and_idempotent(script_name: str):
    ...
    for stage in FULL_STAGE_ORDER[script_name]:
