...
# Deterministic guarded OnLoad/OnActivate handlers (no named states/timers) — a
# full compile of the merged patch is sufficient coverage; see
# repair-papyrus-stubs SKILL.md's dedicated-test-file criteria.
PATCH_CASES = {
    "DefaultActivatorVendorFactionScript": {"onload"},
    "DefaultOneWayContainerScript": {"onload", "onactivate"},
}
...
@pytest.mark.parametrize("script_name", list(PATCH_CASES))
def test_patch_merges_and_compiles_for_fo4(script_name: str):
